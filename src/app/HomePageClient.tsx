...
export default function HomePageClient() {
  ...
  const {
    ...
    teamId: urlTeamId,
    modal: urlModal,
    editProjectId: urlEditProjectId,
    setProjectId,
    setTeamId,
    setModal,
    setEditProjectId
  ...
  const handleSelectedProjectChange = useCallback(
    (projectId: string | null) => {
      setProjectId(projectId, projectId ? 'push' : 'replace')
    },
    [setProjectId]
  )
  ...
  const forgotModalOpen = auth.forgotModalOpen
  const resetModalOpen = auth.resetModalOpen
  const openAuthModal = auth.openAuthModal
  const openForgotModal = auth.openForgotModal
  const openResetModal = auth.openResetModal
  const teamModalOpen = teams.teamModalOpen
  const openTeamModal = teams.openTeamModal
  const providersModalOpen = providers.providersModalOpen
  const openProvidersModal = providers.openProvidersModal
  const addProjectModalOpen = projects.addProjectModalOpen
  const editProjectModalOpen = projects.editProjectModalOpen
  const openAddProjectModal = projects.openAddProjectModal
  const openEditProjectModal = projects.openEditProjectModal
  const savedModalOpen = savedHook.savedModalOpen
  const openSavedModal = savedHook.openSavedModal
  const currentTeamId = auth.currentTeamId
  const authTeams = auth.teams
  const handleTeamSwitch = auth.handleTeamSwitch

  const activeModal = useMemo<ModalState | null>(() => {
    if (resetModalOpen) return 'reset'
    if (forgotModalOpen) return 'forgot'
    if (authModalOpen) return authMode === 'signup' ? 'auth:signup' : 'auth:login'
    if (teamModalOpen) return 'team'
    if (providersModalOpen) return 'providers'
    if (editProjectModalOpen) return 'edit-project'
    if (addProjectModalOpen) return 'add-project'
    if (savedModalOpen) return 'saved'
    return null
  }, [
    addProjectModalOpen,
    authModalOpen,
  ...
  useEffect(() => {
    setModal(activeModal, 'replace')
    if (activeModal === 'edit-project' && projects.editingProject?.id) {
      setEditProjectId(projects.editingProject.id, 'replace')
    } else if (activeModal !== 'edit-project') {
      setEditProjectId(null, 'replace')
    }
  }, [activeModal, projects.editingProject?.id, setEditProjectId, setModal])

  useEffect(() => {
    if (!urlModal) return
    if (urlModal === 'auth:login' && (!authModalOpen || authMode !== 'login')) {
      openAuthModal('login')
    }
    if (urlModal === 'auth:signup' && (!authModalOpen || authMode !== 'signup')) {
      openAuthModal('signup')
    }
    if (urlModal === 'forgot' && !forgotModalOpen) {
      openForgotModal()
    }
    if (urlModal === 'reset' && !resetModalOpen) {
      openResetModal('Reset password')
    }
    if (urlModal === 'team' && !teamModalOpen) {
      void openTeamModal()
    }
    if (urlModal === 'providers' && !providersModalOpen) {
      void openProvidersModal()
    }
    if (urlModal === 'add-project' && !addProjectModalOpen) {
      void openAddProjectModal()
    }
    if (urlModal === 'edit-project' && !editProjectModalOpen && urlEditProjectId) {
      void openEditProjectModal(urlEditProjectId)
    }
    if (urlModal === 'saved' && !savedModalOpen) {
      void openSavedModal()
    }
  }, [
    addProjectModalOpen,
  ...
  useEffect(() => {
    ...
    setTeamId(currentTeamId, 'replace')
  }, [currentTeamId, setTeamId])

  useEffect(() => {
    if (!urlTeamId || !authTeams.length) return
    if (currentTeamId === urlTeamId) return
    if (authTeams.some((team) => team.id === urlTeamId)) {
      handleTeamSwitch(urlTeamId)
    }
  }, [authTeams, currentTeamId, handleTeamSwitch, urlTeamId])
        ...
        {!auth.session ? (
          ...
          <>
            ...
            <ConversationSection
              ...
              completedSteps={projects.completedSteps}
