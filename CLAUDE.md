...
├── server/api.ts         # Request/auth helpers + CORS responses
├── types.ts              # TypeScript interfaces
├── services/
│   ├── agent.ts          # Claude agentic loop orchestration
│   ├── project.ts        # Git clone/sync operations
...
Key tables: `projects`, `conversations`, `teams`, `team_members`, `team_invites`, `git_providers`, `audit_logs`, `shared_conversations`
